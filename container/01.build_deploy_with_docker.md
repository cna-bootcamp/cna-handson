...
- SCG의 config/SecurityConfig에서 allowedOrigins 이해
  Container실행 시 CORS설정을 위해 Frontend의 주소를 환경변수 'spring.cloud.gateway.globalcors.allowedOrgins'에 지정해야 함
  ```
...
```
docker run -d --name mysql-user00 -p 3300:3306 \
--network subride00 \
...
## Run container: sc
  ...
  -e eureka.instance.instanceId=eureka1-user00 \
  ...
  -e eureka.instance.instanceId=eureka2-user00 \
  ...
  --network subride00 \
  -e spring.cloud.gateway.globalcors.allowedOrgins=http://user00.13.215.90.232.nip.io:3000 \
  -e eureka.client.serviceUrl.defaultZone=http://eureka1-user00:8762/eureka,http://eureka2-user00:8762/eureka \
  hiondal/scg:1.0.0
  ```
