...
- Ubuntu설치 
  ...
  ![](./images/2024-12-28-01-29-38.png)

  ![](./images/2024-12-28-01-30-39.png)

  계정명과 암호를 설정합니다.  
  계정명은 'ubuntu'로 합니다.  

- WSL세션 작성 및 로그인 
  ![](./images/2024-12-28-01-33-42.png)
...
- azure Login:   
  ...
  ```
  ubuntu@DESKTOP-PJ5MG3E:~/run-st$ az login --use-device-code
...
- AKS Credential 생성   
  ```
  az configure --defaults group={리소스그룹명}
  az aks get-credentials -n {AKS name} -f ~/.kube/config
  ```
  예시) 
  ```
  az configure --defaults group=tiu-dgga-rg
  az aks get-credentials -n unicorn-aks -f ~/.kube/config 
  ```
...
- ACR로그인
  ...
  로그인
  ```
  docker login <ACR 주소> -u <ACR Name> -p <Password> 
  ```

  ```
  예시)
  docker login unicorncr.azurecr.io -u ondalcr -p NP7Qr6tsbDGHrrNwlxNGmMXqyLbJGPoZBWtC2M2UFo+ACRCa4btE
  ```
...
- (Windows Only) Windows 사용자는 반드시 아래 설정 해야 합니다.
  ...
  또, MobaXTerm에서 아래 package를 설치합니다.   
  ![](./images/images/2024-12-10-00-08-57.png)
  'cygwin'검색하여 install하세요.  
...
- 테스트 
  ...
  > **npx명령 수행 시 에러가 나면 테스트를 Skip하세요**   
  > 개발 시 npx로 기본 구조와 파일을 생성하지 않고 필요한 파일을 직접 만들면 됩니다.  
