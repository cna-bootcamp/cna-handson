...
└── mygrp-infra/
    ...
    │   │   │               │   ├── in/
    │   │   │               │   │   └── web/
    │   │   │               │   │       ├── mygrpController.java
    │   │   │               │   │       └── mygrpControllerHelper.java
    │   │   │               │   └── out/
    │   │   │               │       ├── adapter/
    │   │   │               │       │   └── mygrpProviderImpl.java
    │   │   │               │       ├── entity/
    │   │   │               │       │   └── mygrpEntity.java
    │   │   │               │       ├── feign/
    │   │   │               │       │   └── mygrpFeignClient.java
    │   │   │               │       └── repo/
    │   │   │               │           └── ImygrpRepository.java
    │   │   └── resources/
...
Service 클래스인 **mygrpServiceImpl의 메소드**는 아래와 같습니다.  
- **getmygrpList**   
  구독한 서비스 목록을 리턴 합니다.    
  Controller 레이어에 리턴하기 위해 tomygrpDTO함수로 DTO객체를 만듭니다.  

  ```
    @Override
    public List<mygrpDTO> getmygrpList(String userId) {
        List<mygrp> mygrpList = mygrpProvider.getmygrpList(userId);
        return mygrpList.stream()
                .map(this::tomygrpDTO)
...
- **cancelSub**   
  ...
  public void cancelSub(Long subId, String userId) {
      mygrpProvider.cancelSub(subId, userId);
  }
...
- **subscribeSub**   
  ...
  public void subscribeSub(Long subId, String userId) {
      mygrpProvider.subscribeSub(subId, userId);
  }
  ```
...
- **checkSubscription**   
  ...
  public boolean checkSubscription(String userId, Long subId) {
      return mygrpProvider.isSubscribed(userId, subId);
  }
  ```
...
- 패키지 추가  
  ...
  com.subride.mygrp를 선택하고 우측 마우스 메뉴에서 새로운 패키지를 클릭한 후,     
  패키지명 입력창에 'infra.dto'를 붙여서 생성합니다.   
- DTO 클래스 복사   
  클론 프로젝트의 infra.dto 패키지 밑에 있는 클래스들을 CTRL-c로 복사한 후,      
  개발 프로젝트의 com.subride.mygrp.infra.dto 패키지를 선택하고 붙여넣기 합니다.  
...
- Entity 클래스 복사   
  ...
  **왜냐하면, biz 프로젝트는 infra 프로젝트의 객체들을 참조하면 안되기 때문입니다.**  
  ...
  public class mygrpEntity {
      ...
      public mygrp toDomain() {
          mygrp mygrp = new mygrp();
          mygrp.setUserId(userId);
          mygrp.setSubId(subId);
          return mygrp;
      }
  }
  ```
...
- Entity 클래스 복사   
  ...
  **마이그룹 저장소 인터페이스**
  ...
  ```
  public interface ImygrpRepository extends JpaRepository<mygrpEntity, Long> {
      List<mygrpEntity> findByUserId(String userId);
      Optional<mygrpEntity> findByUserIdAndSubId(String userId, Long subId);
...
- Controller, Gateway 클래스 복사   
  ...
  **Controller -> inport usecase -> service -> outport usecase -> gateway**로    
  ...
  - **getmygrpList: 사용자 가입 구독서비스 목록 리턴**   
    
    Gateway인 mygrpProviderImpl의 getmygrpList는 DB에서 사용자의 구독id 목록을 리턴합니다.    

    ```
      @Override
      public List<mygrp> getmygrpList(String userId) {
          List<mygrpEntity> mygrpEntityList = mygrpRepository.findByUserId(userId);
          return mygrpEntityList.stream()
                  .map(mygrpEntity::toDomain)
                  .collect(Collectors.toList());
      }
    ```

    API가 리턴할 값은 단순히 구독id만이 아니고, 구독서비스의 상세정보도 포함해야 합니다.  
    그래서 mygrpControllerHelper에서는 각 구독목록을 루핑 돌면서 구독추천 서비스에 구독 상세정보를 요청해서   
    리턴할 구동정보 목록을 만들어야 합니다.   
    ```
      public List<mygrpInfoDTO> tomygrpInfoDTOList(List<mygrpDTO> mygrpDTOList) {
          return mygrpDTOList.stream()
                  .map(this::tomygrpInfoDTO)
                  .collect(Collectors.toList());
      }
    ```
    ...
    ```
      public mygrpInfoDTO tomygrpInfoDTO(mygrpDTO mygrpDTO) {
          mygrpInfoDTO mygrpInfoDTO = new mygrpInfoDTO();
          mygrpInfoDTO.setUserId(mygrpDTO.getUserId());
          mygrpInfoDTO.setSubId(mygrpDTO.getSubId());

          ResponseDTO<SubInfoDTO> response = subRecommendFeignClient.getSubDetail(mygrpDTO.getSubId());
          SubInfoDTO subInfoDTO = response.getResponse();

          mygrpInfoDTO.setSubName(subInfoDTO.getSubName());
          mygrpInfoDTO.setCategoryName(subInfoDTO.getCategoryName());
          mygrpInfoDTO.setLogo(subInfoDTO.getLogo());
          mygrpInfoDTO.setDescription(subInfoDTO.getDescription());
          mygrpInfoDTO.setFee(subInfoDTO.getFee());
          mygrpInfoDTO.setMaxShareNum(subInfoDTO.getMaxShareNum());
  ...
  - **cancelSub: 구독취소**   
    ...
    ```
      @Override
      public void cancelSub(Long subId, String userId) {
          mygrpEntity mygrpEntity = mygrpRepository.findByUserIdAndSubId(userId, subId)
                  .orElseThrow(() -> new InfraException("구독 정보가 없습니다."));
          mygrpRepository.delete(mygrpEntity);
      }
    ``` 
  ...
  - **subscribeSub: 구독서비스 가입**    
      ...
      @Override
      public void subscribeSub(Long subId, String userId) {
          mygrpEntity mygrpEntity = mygrpEntity.builder()
                  .userId(userId)
                  .subId(subId)
                  .build();
          mygrpRepository.save(mygrpEntity);
      }
      ```  
  ...
  - **checkSubscription: 구독서비스 가입여부 리턴**    
    ...
    public boolean isSubscribed(String userId, Long subId) {
        return mygrpRepository.existsByUserIdAndSubId(userId, subId);
    }
    ```
...
- API 테스트   
  [마이그룹 서비스 Swagger](http://localhost:18082/swagger-ui/index.html)을 엽니다.   
