...
![alt text](./images/image-eda-1.png)   
...
![alt text](./images/image-eda-2.png)  
...
'sub-dlx-exchange' 중계기는 DLX(Dead Letter Exchange)입니다.   
DLX는 문제가 발생한 이벤트 메시지를 별도의 DLX Queue로 보냅니다.   
...
API의 리턴 객체인 MySubDTO의 필드의 값을 구성할 때, 이 View 모델에서 조회하여 리턴되도록 설계 합니다.  
...
- **구독그룹 서비스 이벤트 처리 클래스 개발**   
  ...
  - **구독그룹 참여 이벤트 처리**   
    ...
    구독정보 view entry가 없으면 새로 생성 합니다.  
    있으면 구독그룹 참여여부를 true로 하고 참여한 구독그룹id를 업데이트 합니다.  
