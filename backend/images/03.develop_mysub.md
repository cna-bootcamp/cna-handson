  ...
  **모든 구독 카테고리 목록을 리턴**합니다.  
  ...
  ```
    ...
    public List<CategoryInfoDTO> getAllCategories() {
        List<Category> categories = mysubProvider.getAllCategories();
  ...
  **소비 카테고리에 맞는 구독 카테고리 리턴**   
  ...
  ```
    ...
    public CategoryInfoDTO getRecommendCategoryBySpending(String userId) {
        ...
        Map<String, Long> spendingByCategory = mysubProvider.getSpendingByCategory(userId);
        ...
        Category category = mysubProvider.getCategoryBySpendingCategory(maxSpendingCategory);
  ...
  **구독 카테고리별 구독 서비스 목록 리턴**   
  ...
  ```
    ...
    public List<SubInfoDTO> getRecommendSubListByCategory(String categoryId) {
        List<Sub> subList = mysubProvider.getSubListByCategoryId(categoryId);
...
com.subride.mysub 패키지 하위에 infra패키지를 추가합니다.  
...
- Entity 클래스 복사   
  ...
  **소비 데이터 저장소**  
    ...
    public interface ISpendingRepository extends JpaRepository<SpendingEntity, Long> {
        ...
        */
        @Query("SELECT new com.subride.mysub.infra.dto.CategorySpendingDTO(s.category, SUM(s.amount)) FROM SpendingEntity s WHERE s.userId = :userId GROUP BY s.category")
...
- 코드 이해하기 
  **Controller -> inport usecase -> service -> outport usecase -> gateway**로 흐름을 따라가면서 코드를 이해합니다.  
  ...
  - 'mysubService.getAllCategories();'의 getAllCategories()위에 커서를 옮기고 CTRL-ALT-b를 누릅니다.   
  - 'mysubProvider.getAllCategories();'의getAllCategories()위에 커서를 옮기고 CTRL-ALT-b를 누릅니다.   
