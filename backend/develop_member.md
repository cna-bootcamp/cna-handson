...
- settings.gradle에 common프로젝트 추가   
    setting.gradle은 멀티 프로젝트에서 하위 모듈(프로젝트)을 관리하는 파일입니다.   
...
- plugins
    ...
    실습에서는 Spring Boot 3.2.6을 사용합니다.   
...
- allprojects 
    ...
    - 라이브러리 정의
        ...
        > **spring-boot-starter-validation**: 유효성 검사 어노테이션(@NotNull, @Valid등)으로 validation check를 지원    
        > **spring-boot-starter-aop**: AOP(Aspect Orientation Programming)라는 방법으로 공통 모듈 작성을 지원(예제에서는 로깅시 사용)        
...
- member-infra 프로젝트의 build.gradle 복사  
    infra프로젝트는 biz프로젝트와 biz프로젝트를 의존하도록 설정함   
    ```
    dependencies {
        implementation project(':common')
